...
async def test_otp_logging_debug_true():
    with patch("app.services.auth_service.logger") as mock_logger, patch.object(settings, "DEBUG", True):
...
async def test_otp_logging_debug_false():
    with patch("app.services.auth_service.logger") as mock_logger, patch.object(settings, "DEBUG", False):
